# app.py  ← FINAL WORKING VERSION (no more errors)
...
st.write("**92%+ Accuracy • Rejects fake/color images**")
st.write("Upload a **grayscale brain MRI** (T1/T2/FLAIR)")
...
if uploaded_file is not None:
    ...
    with st.spinner("Predicting..."):
        ...
        confidence = float(np.max(pred) * 100)  # ← Fixed: convert to Python float

    # Display result
    st.subheader(f"**PREDICTION: {result.upper()}**")
    st.progress(confidence / 100)  # ← Now works perfectly
